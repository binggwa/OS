...
#### 1) Batch processing system (일괄처리 시스템) (1950s~1960s)
...
- 단점
  ...
  - 긴 응답시간
    - 약 6시간 (제출에서 출력까지의 시간)
#### 2) Time-sharing system (시분할 시스템) (60s~70s)
- 여러 사용자가 자원을 동시에 사용
...
### 단일 구조 
- 커널에 모든 기능(메모리 관리자, 프로세서 스케쥴러, 프로세스간 통신, 파일 시스템, 입출력 관리자, 네트워크 관리자)을 모아놓은 구조
- 장점
  - 커널 내 모듈간 직접 통신
    - 효율적 자원 관리 및 사용
- 단점
  - 커널의 거대화
    ...
    - 동일 메모리에 모든 기능이 있어, 한 모듈의 문제가 전체 시스템에 영향 (악성 코드 등)
### 계층 구조
- 단일 구조의 단점을 커버하기 위해 기능을 계층으로 나눠놓음
- 장점
  - 모듈화
    - 계층간 검증 및 수정 용의
  - 설계 및 구현의 단순화
